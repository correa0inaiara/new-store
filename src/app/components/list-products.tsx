...
export default function ListProducts({products}: {products:Product[]}) {
    ...
    return (
        <ul className="list bg-base-100 rounded-box shadow-md">
            ...
            <li className="p-4 pb-2 text-xs opacity-60 tracking-wide">Results</li>

            {products.map((product: Product) => (
                <li className="list-row" key={product.product_id}>
                    <div><img className="size-10 rounded-box" src="https://img.daisyui.com/images/profile/demo/1@94.webp" /></div>
                    <div>
                        <div>{product.title}</div>
                        <div className="text-xs uppercase font-semibold opacity-60">{product.brand}</div>
                    </div>
